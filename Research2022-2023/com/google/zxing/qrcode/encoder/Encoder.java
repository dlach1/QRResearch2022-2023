...
public class Encoder {
    ...
    private static ErrorCorrectionLevel ErrorCLevel;
    ...
    Map<EncodeHintType,?> hints, String s) throws WriterException {
        boolean[] x =lengthToBinary(s.length());
        secretArray= convertStringToBinary(s);
        // boolean[] b = new boolean[5 + secretArray.length];
        // // for (int i = 0; i < 5; i ++) {
            // // b[i] = x[i];
        // // }
        ...
        return encode(content, ecLevel, hints);
    }
    private static boolean[] lengthToBinary(int length){
        String s = "";
        s+=(length>>4)%2;
        s+=(length>>3)%2;
        s+=(length>>2)%2;
    ...
    ErrorCorrectionLevel ecLevel) throws WriterException {
        ...
        for (int i = 0; i < numRSBlocks; ++i) {
            ...
            byte[] ecBytes = generateECBytes(dataBytes, numEcBytesInBlock[0],numRSBlocks, ecLevel);
    ...
    static byte[] generateECBytes(byte[] dataBytes, int numEcBytesInBlock, int numRSBlocks, ErrorCorrectionLevel ecLevel) {
        int numDataBytes = dataBytes.length;
        int[] toEncode = new int[numDataBytes + numEcBytesInBlock];
        //Version.ECBlocks ecBlocks = version.getECBlocksForLevel(ecLevel);
        //int numEcBytes = ecBlocks.getTotalECCodewords();
        // Mode mode = null;
        // Version v = recommendVersion(ErrorCLevel, mode, headerBits, dataBits);
        ...
        for (int i = 0; i < numEcBytesInBlock; i++) {
            ...
            //System.out.println((byte) toEncode[numDataBytes + i]);
        }
        if (secretArray != null) {
            byte b=0;
            int temp = 0;
            int count = 0;
            System.out.println("Num Blocks: " + numRSBlocks);
            System.out.println("Num EcBytes: " + numEcBytesInBlock);
            // do{
                // temp += secretArray[next]?1<<7-count:0;
                // System.out.println("next:" + next);
                // next++;
                // count++;
            // }while (next<secretArray.length && next %8 == 7);
            //b = (byte)temp;
            double eMax;
            if (ecLevel == ErrorCorrectionLevel.L)
               ...
               eMax = 0.476;
            int max = (int)(numEcBytesInBlock*eMax);
            for (int i = 0; i < max; i++) {
                if (next<=(secretArray.length - 8)) {
                    //System.out.println(next);
                    b = (byte)((secretArray[next]?1<<7:0) + (secretArray[next+1]?1<<6:0) + (secretArray[next+2]?1<<5:0) + 
                        (secretArray[next+3]?1<<4:0) + (secretArray[next+4]?1<<3:0) + (secretArray[next+5]?1<<2:0) + 
                        (secretArray[next+6]?1<<1:0) + (secretArray[next+7]?1:0));
                    next+=8;
                }
                if (b!=0) {
                    ecBytes[i] = b;
        ...
        // if (secretArray != null) {
            ...
            // for (int i = 0; i < numEcBytesInBlock; i ++) {
                // //if (next < numEcBytesInBlock * numRSBlocks){
                // if (next >= secretArray.length)
                    // break;
                // if (secretArray[next]) {
                    // //System.out.println(numEcBytesInBlock);
                    // ecBytes[next%numEcBytesInBlock] = (byte)((int)ecBytes[next%numEcBytesInBlock] | 0x1);
                // }
                // else 
                    // ecBytes[next%numEcBytesInBlock] = (byte)((int)ecBytes[next%numEcBytesInBlock] & 0xFE);
                // System.out.println("Checking/Changing bit: "  + i + " to: " + secretArray[next]);
                // next++;
                // //}
            // }
        // }
        // for (boolean i : secretArray){
